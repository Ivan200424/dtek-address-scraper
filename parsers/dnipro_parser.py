"""Парсер аварійних відключень ДТЕК Дніпровські електромережі."""

from parsers.base_parser import BaseParser
from config.regions import REGIONS


class DniproParser(BaseParser):
    """Парсер для ДТЕК Дніпровські електромережі (Дніпропетровська область)."""

    def __init__(self) -> None:
        """Ініціалізація парсера для Дніпропетровської області."""
        region = REGIONS["dnipro"]
        super().__init__("dnipro", region["url"])

    async def parse_outages(self) -> list[dict]:
        """Розпарсити відключення для Дніпропетровської області.
        ...
        try:
            ...
            self.logger.info(
                "Знайдено %d відключень для Дніпра", len(outages)
            )

        except Exception as e:
            self.logger.error("Помилка парсингу Дніпро: %s", e)
